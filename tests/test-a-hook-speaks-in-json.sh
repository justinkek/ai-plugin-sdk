...
# A plugin that only answers tool calls says nothing to a payload that names no
# tool, and has nothing to print at the start of a session either.
if [ -n "$(jq --raw-output '.hooks.SessionStart[]? // empty' "$MANIFEST")" ]; then
  [ "$spoke" -gt 0 ]
